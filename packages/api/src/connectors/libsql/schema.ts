 ...
 * that reached the provider, the embedding -- stored on overflow pages that a
 * table of requests has no use for. Reading fifty rows of a busy agent whole
 * costs hundreds of times what reading their scalars does, and every one of
 * those bytes then crosses the wire and is parsed by a browser that draws
 * none of it.
 *
 ...
 * effort. `json_remove` takes the conversation back out of them, so what
 * comes back is a handful of scalars rather than the messages they sat
 * beside. It is NULL when no provider was asked, which is what
 * `ai_provider_request_log` being NULL already meant.
 *
 ...
 * the agent and ordering by time sorted every row the agent had in a temp
 * B-tree.
 *
 * Appended rather than folded into the initial schema so an existing database
 * gains them by being migrated rather than recreated.
 */
const logSummaries: LibsqlMigration = {
  version: '0005_log_summaries',
  statements: [
    `CREATE VIEW IF NOT EXISTS logs_summary AS
    SELECT
      ...
      -- replaced a response keeps that response on its own log, and the
      -- lists only colour a row by the verdict.
      (SELECT json_group_array(
                json_remove(h.value, '$.request_body', '$.response_body')
              )
       FROM json_each(l.hook_logs) h) AS hook_logs,
      -- The embedding is one of the columns left out, so the filter that asks
      -- for rows that have one is answered here instead of by reading it.
      (l.embedding IS NOT NULL) AS has_embedding,
      json_remove(
        json_extract(l.ai_provider_request_log, '$.request_body'),
        '$.messages', '$.tools', '$.input', '$.prompt'
      ) AS provider_request_params

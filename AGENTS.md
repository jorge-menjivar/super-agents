...
`intent_compaction_model_id`, `intent_compaction_timeout_ms` and
`intent_compaction_reasoning_effort` columns: the routing request waits for
this call, so the agent whose callers send the longest prompts is the one that
needs its own answer -- and it answers for the effort too, unlike the arbiter
below, since the effort that suits a fast model chosen to stop the waiting is
not the one that suits whatever the system compacts with. When the agent has `auto_create_skills` on (the
